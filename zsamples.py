...
class ZSamples:
    ...
    def calculate_scalars(self):
        ...
        z_samples_size = self.z_samples_np.shape[0]
        ...
        for idx in range(z_samples_size):
            for dim in range(z_samples_dimensions):
                ...
                a_n = self.z_samples_np[idx, dim] - min_val
                if a_n == 0.0:
                    less_scalar[idx, dim] = self.outer_level_scalar
                    more_scalar[idx, dim] = 0.0
                    continue
                b_n = max_val - self.z_samples_np[idx, dim]
                if b_n == 0.0:
                    more_scalar[idx, dim] = self.outer_level_scalar
